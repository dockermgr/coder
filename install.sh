...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410081341-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Tuesday, Oct 08, 2024 13:41 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for coder
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
export APPNAME="coder"
export VERSION="202410081341-git"
...
DOCKER_REGISTRY_REPO_NAME="coder"
...
CONTAINER_WEB_SERVER_INT_PORT="7080"
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/coder.script.sh
...
if [ -n "$CODER_USERNAME" ]; then
  CONTAINER_USER_NAME="$CODER_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${CODER_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$CODER_PASSWORD" ]; then
  CONTAINER_USER_PASS="$CODER_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  ...
  CONTAINER_USER_PASS="${CODER_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
